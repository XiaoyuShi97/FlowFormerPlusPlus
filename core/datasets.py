...
class FlyingThings3D_test(FlowDataset):
    def __init__(self, aug_params=None, root='datasets/FlyingThings3D', dstype='frames_cleanpass'):
        super(FlyingThings3D_test, self).__init__(aug_params)

        root = 's3://'

        self.image_list = []
        with open("./flow_dataset/flying_things/test_flyingthings_"+dstype+"_png.txt") as f:
            images = f.readlines()
            for img1, img2 in zip(images[0::2], images[1::2]):
                self.image_list.append([root+img1.strip(), root+img2.strip()])
        self.flow_list = []
        with open("./flow_dataset/flying_things/test_flyingthings_"+dstype+"_pfm.txt") as f:
            flows = f.readlines()
            for flow in flows:
                self.flow_list.append(root+flow.strip())
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if (args.stage == 'chairs') or (args.stage == 'chairs_van'):
    ...
    elif args.stage == 'sintel_finetune':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        sintel_clean = MpiSintel(aug_params, split='training', dstype='clean')
        sintel_final = MpiSintel(aug_params, split='training', dstype='final')  
